...
class Spectrum(object):
    ...
    def __init__(self, x=None, y=None, file=None):
        ...
        #-----------------------------------------------------------------------
        # error: RuntimeWarning: invalid value encountered in true_divide

        pdf  = cdf/max(cdf)
        if max(cdf)==0:
            print('Spectrum from %s contains no data'%(str(file)))
            print('x = '+str(self.x))
            print('y = '+str(self.y))
            print('cdf = '+str(cdf))
            print('pdf = '+str(pdf))
    ...
    def trace(self, photon, free_pathlength):
        ...
        if (sampled_pathlength < free_pathlength):
            ...
            if (np.random.uniform() < material.quantum_efficiency):
                ...
                #print "   * Photon Lost *"
                #Emission does not occur. Now set active = False and return
                photon.active = False
                return photon
